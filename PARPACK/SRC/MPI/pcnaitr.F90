...
!
!  where OP and B are as in pcnaupd.  The B-norm of r_{k+p} is also
...
!          semi-inner product for the operator OP.  See pcnaupd.
...
!
!  WORKL   Complex work space used for Gram Schmidt orthogonalization
!
!  INFO    Integer.  (OUTPUT)
!          = 0: Normal exit.
...
!
!\Routines called:
!     pcgetv0   Parallel ARPACK routine to generate the initial vector.
...
!     cdotc     Level 1 BLAS that computes the scalar product of
!               two vectors.
!     cscal     Level 1 BLAS that scales a vector.
!     csscal    Level 1 BLAS that scales a complex vector by a real number.
!     pscnorm2  Parallel version of Level 1 BLAS that computes the
!               norm of a vector.
!
!\Author
...
!        p_{j} = p_{j}/betaj
!     3) r_{j} = OP*v_{j} where OP is defined as in pcnaupd
