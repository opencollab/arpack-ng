...
!
!  psnaupd is usually called iteratively to solve one of the
!  following problems:
!
!  Mode 1:  A*x = lambda*x.
!           ===> OP = A  and  B = I.
...
!          call to psnaupd.  IDO will be set internally to
!          indicate the type of operation to be performed.  Control is
!          then given back to the calling routine which has the
!          responsibility to carry out the requested operation and call
!          psnaupd with the result.  The operand is given in
...
!          Must be 1,2,3,4; See under \Description of psnaupd for the
!          four modes available.
!
!          IPARAM(8) = NP
!          When ido = 3 and the user provides shifts through reverse
!          communication (IPARAM(1)=0), psnaupd returns NP, the number
...
!     completion of psnaupd. This is new starting with release 2 of ARPACK.
!
...
!
!\Routines called:
!     psnaup2  Parallel ARPACK routine that implements the Implicitly Restarted
!              Arnoldi Iteration.
...
!        | The final workspace is needed by subroutine psneigh called  |
!        | by psnaup2. Subroutine sneigh calls LAPACK routines for     |
