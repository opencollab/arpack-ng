...
!
!\Description:
!  Intermediate level interface called by pznaupd.
!
!\Usage:
!  call pznaup2
...
!\Arguments
!
!  COMM, IDO, BMAT, N, WHICH, NEV, TOL, RESID: same as defined in pznaupd.
!  MODE, ISHIFT, MXITER: see the definition of IPARAM in pznaupd.
!
...
!          See Data Distribution Note in PZNAUPD.
!
...
!
!\Routines called:
!     pzgetv0  Parallel ARPACK initial vector generation routine.
!     pznaitr  Parallel ARPACK Arnoldi factorization routine.
!     pznapps  Parallel ARPACK application of implicit shifts routine.
!     pzneigh  Parallel ARPACK compute Ritz values and error bounds routine.
!     pzngets  Parallel ARPACK reorder Ritz values and error bounds routine.
...
!        | to the shift application routine pznapps.                 |
!        %-----------------------------------------------------------%
!
         np  = kplusp - nev
!
...
!        | Ritz estimates obtained from pzneigh.            |
!        %--------------------------------------------------%
!
...
!        | the first step of the next call to pznaitr. |
!        %---------------------------------------------%
!
         cnorm = .true.
         call arscnd (t2)
