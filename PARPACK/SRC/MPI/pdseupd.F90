...
!          the Ritz values of OP computed by pdsaupd transformed to
...
!     pdnorm2 Parallel ARPACK routine that computes the 2-norm of a vector.
...
!     |       pdsaupd stores the B-norm of the last residual  |
...
!     |       The wanted values are in the first NCONV spots. |
!     | NOTE: workl(1:4*ncv) is set by pdsaupd and is not     |
!     |       modified by pdseupd.                            |
!     %-------------------------------------------------------%
!
!     %-------------------------------------------------------%
!     | The following is used and set by pdseupd.             |
...
!     |       Workspace. Needed by dsteqr and by pdseupd.    |
...
!     | Upon exit of pdsaupd WORKD(1:N) has   |
!     | B*RESID(1:N).                         |
!     %---------------------------------------%
!
      rnorm = workl(ih)
...
!        |    where the theta are the Ritz values returned by pdsaupd. |
!        | NOTES:                                                      |
