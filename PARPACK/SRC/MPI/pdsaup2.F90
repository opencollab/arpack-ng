...
!
!\Description:
!  Intermediate level interface called by pdsaupd.
!
!\Usage:
!  call pdsaup2
...
!\Arguments
!
!  COMM, IDO, BMAT, N, WHICH, NEV, TOL, RESID: same as defined in pdsaupd.
!  MODE, ISHIFT, MXITER: see the definition of IPARAM in pdsaupd.
!
...
!          of H starting at H(1,2). If pdsaup2 converges store the
...
!          See Data Distribution Note in pdsaupd.
!
...
!     pdsaitr  Parallel ARPACK Lanczos factorization routine.
!     pdsapps  Parallel ARPACK application of implicit shifts routine.
!     dsconv   ARPACK convergence of Ritz values routine.
!     pdseigt  Parallel ARPACK compute Ritz values and error bounds routine.
!     pdsgets  Parallel ARPACK reorder Ritz values and error bounds routine.
...
!
      ...
      integer    iter, j, kplusp, msglvl, nconv, nevbef, nev0,&
...
!
!        %-----------------------------------------------------%
!        | pdsaitr was unable to build an Lanczos factorization|
...
!        | in the exact shift case, pdsgets already handles this.  |
!        %---------------------------------------------------------%
!
...
!        | After pdsapps is done, we have a Lanczos                |
...
!        | the first step of the next call to pdsaitr. |
!        %---------------------------------------------%
!
         cnorm = .true.
         call arscnd (t2)
