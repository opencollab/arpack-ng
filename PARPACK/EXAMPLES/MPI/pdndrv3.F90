...
!
      ...
      if ( myid .lt. nprocs-1 ) then
         tag = myid+1
         call mpi_send( v(nloc), cnt, MPI_DOUBLE_PRECISION,&
                        next, tag, comm, ierr )
      endif
      if ( myid .gt. 0 ) then
         call mpi_recv( mv_buf, cnt, MPI_DOUBLE_PRECISION, prev, myid,&
                        comm, status, ierr )
         w(1) = w(1) + dl*mv_buf
      endif
!
      ...
      if ( myid .lt. nprocs-1 ) then
         call mpi_recv( mv_buf, cnt, MPI_DOUBLE_PRECISION, next, myid,&
                        comm, status, ierr )
