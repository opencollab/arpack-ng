...
!
      ...
      if ( myid .lt. nprocs-1 ) then
         tag = myid+1
         call mpi_send( v(nloc), start, MPI_REAL,&
                        next, tag, comm, ierr )
      endif
      if ( myid .gt. 0 ) then
         call mpi_recv( mv_buf, start, MPI_REAL, prev, myid,&
                        comm, status, ierr )
         w(1) = w(1) + dl*mv_buf
      endif
!
      if ( myid .gt. 0 ) then
         tag = myid-1
         call mpi_send( v(1), start, MPI_REAL,&
                        prev, tag, comm, ierr )
      endif
      if ( myid .lt. nprocs-1 ) then
         call mpi_recv( mv_buf, start, MPI_REAL, next, myid,&
                        comm, status, ierr )
