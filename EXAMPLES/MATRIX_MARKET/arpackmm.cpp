...
class options {
  public:
    ...
    int readCmdLine(int argc, char ** argv) {
      ...
      for (int a = 1; argv && a < argc; a++) {
        ...
        if (clo == "--slvItrTol") {
          ...
          stringstream t(argv[a]);
          double tol = 0.;
          t >> slvItrTol; if (!t) {cerr << "Error: bad " << clo << " - bad argument" << endl; return usage();}
        }
        if (clo == "--slvItrMaxIt") {
          a++; if (a >= argc) {cerr << "Error: bad " << clo << " - need argument" << endl; return usage();}
          stringstream mi(argv[a]);
          int maxIt = 0;
          mi >> slvItrMaxIt; if (!mi) {cerr << "Error: bad " << clo << " - bad argument" << endl; return usage();}
        }
        if (clo == "--slvItrPC") {
          a++; if (a >= argc) {cerr << "Error: bad " << clo << " - need argument" << endl; return usage();}
        ...
        if (clo == "--slvDrtPivot") {
          ...
          pv >> slvDrtPivot; if (!pv) {cerr << "Error: bad " << clo << " - bad argument" << endl; return usage();}
        }
        if (clo == "--slvDrtOffset") {
          a++; if (a >= argc) {cerr << "Error: bad " << clo << " - need argument" << endl; return usage();}
          stringstream of(argv[a]);
          of >> slvDrtOffset; if (!of) {cerr << "Error: bad " << clo << " - bad argument" << endl; return usage();}
        }
        if (clo == "--slvDrtScale") {
          a++; if (a >= argc) {cerr << "Error: bad " << clo << " - need argument" << endl; return usage();}
          stringstream sc(argv[a]);
          sc >> slvDrtScale; if (!sc) {cerr << "Error: bad " << clo << " - bad argument" << endl; return usage();}
        }
    ...
    int usage(int rc = 1) {
      ...
      cout << "                      if q_j is the j-th Schur vector, then A x q_j is a linear combination of q_1, ..., q_j." << endl;
      ...
      cout << "                      LU:   direct method, any matrices (pivoting needed)" << endl;
      cout << "                      QR:   direct method, any matrices (pivoting needed)" << endl;
      cout << "                      LLT:  direct method, SPD matrices only (pivoting not needed)" << endl;
      cout << "                      LDLT: direct method, symmetric positive semi-definite matrices only (pivoting not needed)" << endl;
      ...
      cout << "                    default: 0" << endl;
      cout << "  --restart:        restart from previous run (which had produced arpackSolver.*.out)." << endl;
...
ostream & operator<< (ostream & ostr, options const & opt) {
  ...
  ostr << "OPT: slv " << opt.slv << ", slvItrPC " << opt.slvItrPC << ", slvItrTol " << opt.slvItrTol;
  ostr << ", slvItrMaxIt " << opt.slvItrMaxIt << ", slvDrtPivot " << opt.slvDrtPivot;
  ostr << ", slvDrtOffset " << opt.slvDrtOffset << ", slvDrtScale " << opt.slvDrtScale << endl;
  ostr << "OPT: check " << (opt.check ? "yes" : "no") << ", verbose " << opt.verbose << ", debug " << opt.debug;
...
int itrSolve(options & opt, output & out) {
  ...
  if (slvItrPC == "ILU") {
    ...
    stringstream dt(dropTol); dt >> slvItrILUDropTol;
  }

  int slvItrILUFillFactor = 2;
  if (slvItrPC == "ILU") {
    string fillFactor; getline(clo, fillFactor);
    stringstream ff(fillFactor); ff >> slvItrILUFillFactor;
  }
...
int main(int argc, char ** argv) {
  ...
  cout << endl;
  cout << "OUT: mode " << out.mode << ", nb EV found " << out.nbVal << ", nb iterations " << out.nbIt << endl;
